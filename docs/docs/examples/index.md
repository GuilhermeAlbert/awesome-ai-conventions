---
title: "Examples"
sidebar_position: 1
---

# Examples

Every page below is generated from the repository's `examples/**` files. Edit the source example, then run `npm run generate` from `docs/` to refresh this section.

## How To Use These Examples

- Copy examples as starting points, not as universal templates.
- Keep only fields that are true for your project.
- Prefer the shortest useful file before adding team-specific policy.
- Update the source file in `examples/**`; generated docs are overwritten.

- [`agent-cards/.well-known/agent-card.json`](/docs/examples/generated/agent-cards/well-known/agent-card)
- [`agents-md/AGENTS.md`](/docs/examples/generated/agents-md/AGENTS)
- [`ai-plugin/.well-known/ai-plugin.json`](/docs/examples/generated/ai-plugin/well-known/ai-plugin)
- [`ai-plugin/openapi.yaml`](/docs/examples/generated/ai-plugin/openapi)
- [`aiignore/.aiignore`](/docs/examples/generated/aiignore)
- [`auth-md/auth.md`](/docs/examples/generated/auth-md/auth)
- [`claude-md/CLAUDE.md`](/docs/examples/generated/claude-md/CLAUDE)
- [`design-md/DESIGN.md`](/docs/examples/generated/design-md/DESIGN)
- [`eval-yaml/EVAL.yaml`](/docs/examples/generated/eval-yaml/EVAL)
- [`gemini-md/GEMINI.md`](/docs/examples/generated/gemini-md/GEMINI)
- [`llms-txt/llms-full.txt`](/docs/examples/generated/llms-txt/llms-full)
- [`llms-txt/llms.txt`](/docs/examples/generated/llms-txt/llms)
- [`memory-bank/.roo/activeContext.md`](/docs/examples/generated/memory-bank/roo/activeContext)
- [`memory-bank/cline_docs/activeContext.md`](/docs/examples/generated/memory-bank/cline_docs/activeContext)
- [`memory-bank/cline_docs/productContext.md`](/docs/examples/generated/memory-bank/cline_docs/productContext)
- [`memory-bank/cline_docs/systemArchitecture.md`](/docs/examples/generated/memory-bank/cline_docs/systemArchitecture)
- [`memory-md/MEMORY.md`](/docs/examples/generated/memory-md/MEMORY)
- [`model-context-protocol/claude_desktop_config.json`](/docs/examples/generated/model-context-protocol/claude_desktop_config)
- [`plan-md/PLAN.md`](/docs/examples/generated/plan-md/PLAN)
- [`pricing-md/pricing.md`](/docs/examples/generated/pricing-md/pricing)
- [`prompt-files/refactor.prompt`](/docs/examples/generated/prompt-files/refactor)
- [`prompt-files/system_prompt.txt`](/docs/examples/generated/prompt-files/system_prompt)
- [`prompty/summarize_release_notes.prompty`](/docs/examples/generated/prompty/summarize_release_notes)
- [`skill-md/SKILL.md`](/docs/examples/generated/skill-md/SKILL)
- [`tool-specific-rules/.clinerules`](/docs/examples/generated/tool-specific-rules/clinerules)
- [`tool-specific-rules/.cursor/rules/project.mdc`](/docs/examples/generated/tool-specific-rules/cursor/rules/project)
- [`tool-specific-rules/.github/copilot-instructions.md`](/docs/examples/generated/tool-specific-rules/github/copilot-instructions)
